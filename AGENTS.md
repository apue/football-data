...
Use `scripts/run_editorial_queue.py` as the default publication entrypoint. It rebuilds `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the Agents SDK-backed editorial state graph for pending dates when credentials exist, writes `manifests/editorial-run.json`, and rebuilds the homepage. For a targeted local date, use `scripts/run_editorial_agent.py --date YYYY-MM-DD`. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.
