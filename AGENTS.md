...
Default Editor's Choices scoring uses `config/scoring/v0.3.json`. The impact layer is derived from PMSR goals, lineup status, and final scoreline only; it can reward opening, equalizing, go-ahead, match-winning, late, stoppage-time, late match-winning goals, only-goal winners, braces, hat-tricks, and substitute scoring bursts. Do not use POTM labels, media ratings, or social reactions as direct scoring inputs.
