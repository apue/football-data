...
Use `scripts/run_editorial_loop.py --date YYYY-MM-DD` as the default publication entrypoint. It runs the explicit review-repair-validate loop, writes audit traces under `agent-runs/`, and calls the deterministic generator underneath. Use `reports/editorial/YYYY-MM-DD.md` as the human review surface. `scripts/generate_editorial.py` remains the low-level evidence/draft generator; it produces `evidence.json`, `fact_bank.zh.json`, `brief.zh.json`, `brief.en.json`, and a Markdown draft brief, not publishable copy. Write Chinese from `fact_bank.zh.json` as a from-scratch Chinese sports editor. Use `brief.zh.json` only as a legacy/diagnostic artifact, not as the primary Chinese writing input. Write English from `brief.en.json` and `evidence.json`; do not use either finished language version as input for the other. Review Chinese with a strict `qu-ai-wei` style pass for translationese, empty polish, and factual drift; use a `humanizer-zh` style repair only for cards that fail review. Do not ask the user to review compiled JSON; regenerate frontend JSON/HTML with `scripts/render_editorial.py` after Markdown edits.

Use `scripts/run_editorial_queue.py` for autonomous daily publishing. It reads `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the editor agent for pending dates when credentials exist, validates with `scripts/run_editorial_loop.py --use-existing-markdown`, and writes `manifests/editorial-run.json`. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.
...
Current POTM calibration defaults to `config/scoring/v0.3.json`. Use `--scoring-config config/scoring/v0.1.json` or `--scoring-config config/scoring/v0.2.json` only to reproduce or explain historical ranking misses.
