...
Use `scripts/run_editorial_queue.py` as the default publication entrypoint. It rebuilds `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the Agents SDK-backed editorial state graph for pending dates when credentials exist, writes `manifests/editorial-run.json`, and rebuilds the homepage. Per-card writer/editor calls retry according to `EDITORIAL_AGENT_MAX_ATTEMPTS`; if one card still fails, keep publishing the date with the latest available draft for that card and record the warning in the run audit. For a targeted local date, use `scripts/run_editorial_agent.py --date YYYY-MM-DD`. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.
