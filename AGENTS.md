...
Use `scripts/run_editorial_queue.py` as the default publication entrypoint. It rebuilds `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the editor agent for pending dates when credentials exist, writes `manifests/editorial-run.json`, and rebuilds the homepage. For a targeted local date, use `scripts/run_editorial_agent.py --date YYYY-MM-DD`. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.

The editor agent produces `evidence.json`, `fact_bank.zh.json`, `brief.en.json`, `reports/editorial/YYYY-MM-DD.md`, compiled `choices.json`, and an `agent-runs/YYYY-MM-DD.json` audit. Write Chinese from `fact_bank.zh.json` as a from-scratch Chinese sports editor. Write English from `brief.en.json` and `evidence.json`; do not use either finished language version as input for the other. Review Chinese with a strict `qu-ai-wei` style pass for translationese, empty polish, and factual drift; use a `humanizer-zh` style repair only for cards that fail review. Do not reintroduce `brief.zh.json` or manual render/generate entrypoints.
