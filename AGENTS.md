...
Use `scripts/run_editorial_queue.py` as the default publication entrypoint. It rebuilds `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the Agents SDK-backed editorial state graph for pending dates when credentials exist, writes `manifests/editorial-run.json`, and rebuilds the homepage. The automatic queue prioritizes only the latest data match date; older hash changes are recorded as stale and do not block the newest day. For a targeted backfill date, use `scripts/run_editorial_queue.py --date YYYY-MM-DD`. Per-card editor-agent calls use `EDITORIAL_AGENT_MAX_CONCURRENCY` and `EDITORIAL_AGENT_MAX_ATTEMPTS`; if one card still fails, keep publishing the date with static fallback copy for that card and record the warning in the run audit. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.

The editor agent produces `evidence.json`, `fact_bank.zh.json`, `brief.en.json`, `reports/editorial/YYYY-MM-DD.md`, `reports/editorial/YYYY-MM-DD.pre-review.md`, compiled `choices.json`, and an `agent-runs/YYYY-MM-DD.json` audit. The workflow order is evidence -> optional research -> zh editor agent -> en editor agent -> render/only-if-needed repair -> deterministic validation -> compile/publish. Write Chinese from `fact_bank.zh.json` as a from-scratch Chinese sports editor. Write English from `brief.en.json` and `evidence.json`; do not use either finished language version as input for the other. For local Codex review, write structured comments and pass them with `--review-feedback`, capped by `--max-review-loops`; this is an extra targeted repair pass, not the default publication path. Keep this single compact graph as the only supported editorial workflow.
