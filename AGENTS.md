...
The active production experiment is `ai_rerank_guardrails_v2`, using the `ai_rerank_selection_v1` workflow variant. Deterministic scoring builds a rich candidate pool, the local AI editor reranks only that pool and explains both selected and skipped higher-ranked candidates, deterministic validation enforces pool membership and skip explanations, then local copy is written from the selected evidence packets. Public output is compiled `choices.json`, dated/index editorial HTML, and the homepage cards. Local audit output lives under `agent-runs/YYYY-MM-DD/` as `rankings.json`, `candidate_pool.json`, `selector_input.json`, `selection_decision.json`, `selection_validation.json`, `copy.json`, and `run.json`. `scripts/run_editorial_queue.py` remains available for manual smoke tests or legacy OpenAI Agents SDK runs; `scripts/run_editorial_v2.py --date YYYY-MM-DD` is for low-level debugging only.
