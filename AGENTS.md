...
Use `scripts/run_editorial_queue.py` as the default publication entrypoint. It rebuilds `manifests/editorial-queue.json`, skips safely with `needs_credentials` when OpenAI credentials are missing, runs the active v2 editorial experiment for pending dates when credentials exist, writes `manifests/editorial-run.json` and `manifests/editorial-v2-run.json`, and rebuilds the homepage. The automatic queue prioritizes only the latest data match date; older hash changes are recorded as stale and do not block the newest day. For a targeted backfill date, use `scripts/run_editorial_queue.py --date YYYY-MM-DD`. Agent calls use `EDITORIAL_AGENT_MAX_CONCURRENCY` and `EDITORIAL_AGENT_MAX_ATTEMPTS`. GitHub Actions injects credentials from repository secrets; local development can copy `.env.example` to `.env.local`.

The active production mode is `ai_rerank_selection_v1`. Deterministic scoring builds a rich candidate pool, the AI selection editor reranks only that pool and explains both selected and skipped higher-ranked candidates, deterministic validation enforces pool membership and skip explanations, then separate Chinese and English copy editor calls write from the selected evidence packets. Public output is compiled `choices.json`, dated/index editorial HTML, and the homepage cards. Local audit output lives under `agent-runs/YYYY-MM-DD/` as `rankings.json`, `candidate_pool.json`, `selector_input.json`, `selection_decision.json`, `selection_validation.json`, and `run.json`. Low-level debugging uses `scripts/run_editorial_v2.py --date YYYY-MM-DD`.
